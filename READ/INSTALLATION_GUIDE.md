...
4. Create a dedicated user for the application:
   ```sql
   CREATE USER school_app WITH ENCRYPTED PASSWORD 'your_password';
   ```
5. Grant privileges to the user:
   ```sql
   GRANT ALL PRIVILEGES ON DATABASE school_management TO school_app;
   ```
6. Exit PostgreSQL:
   ```
   \q
...
1. Connect to the database:
   ```
   psql -U school_app -d school_management
   ```
...
- `DB_URL`: JDBC URL for the PostgreSQL database
  ...
  - Example: `jdbc:postgresql://localhost:5432/school_management`
- `DB_USER`: Database username (e.g., `school_app`)
...
- `PORT`: (Optional) Port number for the web server (default: 8080)
...
2. Set environment variables:
   ```
   setx DB_URL "jdbc:postgresql://localhost:5432/school_management" /M
   setx DB_USER "school_app" /M
   setx DB_PASSWORD "your_password" /M
...
3. Run the application:
   ...
   ```
   java -DPORT=8081 -jar school-management-system.jar
   ```
